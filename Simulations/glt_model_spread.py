...
trace = open('Code/Traces/glt_model_spread.txt', 'w')
...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(argv, 'hT:s:g:k:A:Q:')
    except getopt.GetoptError:
        print('glt_model_spread.py -T -s -g -k -A -Q')
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-h':
    ...
    trace.write(f'Model parameters: T={T}, sigma={sigma}, gamma={gamma}, k={k}, A={A}, Q={Q}\n')
    trace.write('-' * 100 + '\n')
    ...
    bid_spread_aysm = 1 / gamma * np.log(1 + gamma / k) + (2 * inventory + 1) / 2 * np.sqrt((sigma**2 * gamma) / (2 * k * A) * (1 + gamma / k)**(1 + k / gamma))
    ask_spread_aysm = 1 / gamma * np.log(1 + gamma / k) - (2 * inventory - 1) / 2 * np.sqrt((sigma**2 * gamma) / (2 * k * A) * (1 + gamma / k)**(1 + k / gamma))
    ...
    # fig.text(0.5, 0.05, caption, ha='center')
    ax.view_init(30, 30)
    plt.savefig('Plots/ask_spread.png', bbox_inches='tight', dpi=100, format='png')

    t = np.arange(T)
    t, q = np.meshgrid(t, inventory[6:-6])

    # Plot of the bid-ask spread
    fig, ax = plt.subplots(subplot_kw={'projection': '3d'})
    surf    = ax.plot_surface(t, q, bid_ask_spread[:, 6:-6].T, color='firebrick', alpha=0.75)
    ax.set_xlabel('Time (Sec)')
    ax.set_ylabel('Inventory')
    ...
    # fig.text(0.5, 0.05, caption, ha='center')
    ax.view_init(30, 210)
    plt.savefig('Plots/bid_ask_spread.png', bbox_inches='tight', dpi=100, format='png')
    ...
    plt.savefig('Plots/bid_spread_aysm.png', bbox_inches='tight', dpi=100, format='png')
    ...
    # plt.figtext(0.5, 0, caption, ha='center')
    plt.savefig('Plots/ask_spread_aysm.png', bbox_inches='tight', dpi=100, format='png')

    # Save the optimal quotes as CSV files
    pd.DataFrame(bid_spread).to_csv('Code/Results/bid_spread.csv')
    pd.DataFrame(ask_spread).to_csv('Code/Results/ask_spread.csv')
    pd.DataFrame(bid_ask_spread).to_csv('Code/Results/bid_ask_spread.csv')
