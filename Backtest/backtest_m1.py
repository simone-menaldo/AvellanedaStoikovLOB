...
trace = open('Code/Traces/backtest_m1.txt', 'w')
...
def main(argv):
    ...
    date_range = pd.bdate_range('2015-01-01', '2015-01-30')
    ...
    table = open('Code/Results/backtest_table_m1.txt', 'w')
    ...
    # Variables holding inventory, P&L and MM spread
    inventory = [q0]
    pnl       = [0]
    ...
    for d in days_range[est_days:]:
        ...
        if end_strat == 0 and symmetry == 1:
        ...
        elif end_strat == 0 and symmetry == 0:

            # Asymptotic bid and ask spreads
            bid_spread_aysm = 1 / gamma * np.log(1 + gamma / k_bid) + (2 * inventory[-1] + 1) / 2 * np.sqrt((sigma**2 * gamma) / (2 * k_bid * A_bid) * (1 + gamma / k_bid)**(1 + k_bid / gamma))
            ask_spread_aysm = 1 / gamma * np.log(1 + gamma / k_ask) - (2 * inventory[-1] - 1) / 2 * np.sqrt((sigma**2 * gamma) / (2 * k_ask * A_ask) * (1 + gamma / k_ask)**(1 + k_ask / gamma))
        ...
        elif end_strat == 1 and symmetry == 0:

            # Computation of alpha and eta
            alpha_bid = k_bid / 2 * gamma * sigma**2
            eta_bid   = A_bid * (1 + gamma / k_bid)**(-(1 + k_bid / gamma))

            alpha_ask = k_ask / 2 * gamma * sigma**2
            eta_ask   = A_ask * (1 + gamma / k_ask)**(-(1 + k_ask / gamma))

            # Computation of the matrix M for the bid side
            up_diag_bid  = alpha_bid * (Q - np.arange(0, Q + 1))**2
            low_diag_bid = alpha_bid * (Q - np.arange(0, Q)[::-1])**2
            M_bid        = np.diag(np.concatenate((up_diag_bid, low_diag_bid)))
            for i in range(M_bid.shape[0]):
                for j in range(M_bid.shape[1]):
                    if j == (i + 1) or j == (i - 1):
                        M_bid[i, j] = -eta_bid

            # Computation of the matrix M for the ask side
            up_diag_ask  = alpha_ask * (Q - np.arange(0, Q + 1))**2
            low_diag_ask = alpha_ask * (Q - np.arange(0, Q)[::-1])**2
            M_ask        = np.diag(np.concatenate((up_diag_ask, low_diag_ask)))
            for i in range(M_ask.shape[0]):
                for j in range(M_ask.shape[1]):
                    if j == (i + 1) or j == (i - 1):
                        M_ask[i, j] = -eta_ask

            # Computation of the v(t) function
            v_bid = pd.Series(expm(-M_bid * T_end) @ np.ones(2 * Q + 1).T, index=['{}'.format(j) for j in np.arange(-Q, Q+1)])
            v_ask = pd.Series(expm(-M_ask * T_end) @ np.ones(2 * Q + 1).T, index=['{}'.format(j) for j in np.arange(-Q, Q+1)])

            # Bid and ask spreads
            if inventory[-1] != Q:
                bid_spread = 1 / k_bid * np.log(v_bid[f'{inventory[-1]}'] / v_bid[f'{inventory[-1] + 1}']) + 1 / gamma * np.log(1 + gamma / k_bid)
            if inventory[-1] != -Q:
                ask_spread = 1 / k_ask * np.log(v_ask[f'{inventory[-1]}'] / v_ask[f'{inventory[-1] + 1}']) + 1 / gamma * np.log(1 + gamma / k_ask)
            ...
            for i in tqdm(range(trades.shape[0])):
                ...
                if (trade_time - ref_time) >= T:
                    ...
                    v_bid = pd.Series(expm(-M_bid * (trade_time - strat_time)) @ np.ones(2 * Q + 1).T, index=['{}'.format(j) for j in np.arange(-Q, Q+1)])
                    v_ask = pd.Series(expm(-M_ask * (trade_time - strat_time)) @ np.ones(2 * Q + 1).T, index=['{}'.format(j) for j in np.arange(-Q, Q+1)])
    ...
    plt.savefig(f'Plots/backtest_m1_inv_{end_strat}_{symmetry}.png', bbox_inches='tight', dpi=100, format='png')
    ...
    plt.grid()
    ax.set_facecolor('#f2f2f2')
    plt.savefig(f'Plots/backtest_m1_pnl_{end_strat}_{symmetry}.png', bbox_inches='tight', dpi=100, format='png')
    ...
    plt.grid()
    ax.set_facecolor('#f2f2f2')
    plt.savefig(f'Plots/backtest_m1_prices_{end_strat}_{symmetry}.png', bbox_inches='tight', dpi=100, format='png')
