...
trace = open('Code/Traces/glt_model_spread_drift.txt', 'w')

def main(argv):

    # Import global variables
    global T, mu, sigma, gamma, k, A, Q

    # Define script parameters
    try:
        opts, args = getopt.getopt(argv, 'hT:m:s:g:k:A:Q:')
    except getopt.GetoptError:
        print('glt_model_spread_drift.py -T -m -s -g -k -A -Q')
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-h':
    ...
    trace.write(f'Model parameters: T={T}, mu={mu}, sigma={sigma}, gamma={gamma}, k={k}, A={A}, Q={Q}\n')
    trace.write('-' * 100 + '\n')
    ...
    bid_spread_aysm = 1 / gamma * np.log(1 + gamma / k) + (- mu / (gamma * sigma**2) + (2 * inventory + 1) / 2) * np.sqrt((sigma**2 * k) / (2 * k * A) * (1 + gamma / k)**(1 + k / gamma))
    ask_spread_aysm = 1 / gamma * np.log(1 + gamma / k) + (  mu / (gamma * sigma**2) - (2 * inventory - 1) / 2) * np.sqrt((sigma**2 * k) / (2 * k * A) * (1 + gamma / k)**(1 + k / gamma))
    ...
    # caption = f'Behaviour of the optimal bid quote with time and inventory:\n\u03BC={mu}, \u03C3={sigma}, A={A}, k={k}, \u03B3={gamma}, T={T}'
    # fig.text(0.5, 0.05, caption, ha='center')
    ax.view_init(30, -30)
    plt.savefig('Plots/bid_spread_drift.png', bbox_inches='tight', dpi=100, format='png')
    ...
    # fig.text(0.5, 0.05, caption, ha='center')
    ax.view_init(30, 30)
    plt.savefig('Plots/ask_spread_drift.png', bbox_inches='tight', dpi=100, format='png')
    ...
    # fig.text(0.5, 0.05, caption, ha='center')
    ax.view_init(30, 210)
    plt.savefig('Plots/bid_ask_spread_drift.png', bbox_inches='tight', dpi=100, format='png')
    ...
    plt.savefig('Plots/bid_spread_aysm_drift.png', bbox_inches='tight', dpi=100, format='png')
    ...
    # plt.figtext(0.5, 0, caption, ha='center')
    plt.savefig('Plots/ask_spread_aysm_drift.png', bbox_inches='tight', dpi=100, format='png')
